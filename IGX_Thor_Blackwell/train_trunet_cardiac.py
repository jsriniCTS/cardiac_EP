...
# --------------------------------------------------------------------------- #
def report_device(preferred=None):
    """Enumerate visible CUDA devices and select one.

    IGX Thor exposes TWO devices: cuda:0 = integrated Thor iGPU, cuda:1 = the
    discrete RTX PRO 6000 Blackwell. Blindly using cuda:0 would train on the
    weak iGPU, so by default we auto-pick the discrete card (name contains
    'RTX'/'6000' and not 'Thor'). Override with --gpu-index, or by exporting
    CUDA_VISIBLE_DEVICES (which remaps indices).
    """
    ...
    for i in range(n):
        ...
        infos.append((i, p.name, p.major, p.minor, p.total_memory / (1024 ** 3)))
        print(f"[thor] cuda:{i}  {p.name}  sm_{p.major}{p.minor}  {p.total_memory/(1024**3):.0f} GB")

    if preferred is not None:
        if not (0 <= preferred < n):
            print(f"[thor] *** --gpu-index {preferred} out of range (0..{n-1}); auto-selecting. ***")
            preferred = None
    if preferred is not None:
        idx = preferred
    else:
        discrete = [t for t in infos
                    if ("rtx" in t[1].lower() or "6000" in t[1].lower())
                    and "thor" not in t[1].lower()]
        if discrete:
            idx = discrete[0][0]
        else:  # fall back to highest compute capability, then most memory
            idx = max(infos, key=lambda t: (t[2], t[3], t[4]))[0]

    sel = infos[idx]
    torch.cuda.set_device(idx)
    print(f"[thor] selected   : cuda:{idx}  {sel[1]}  sm_{sel[2]}{sel[3]}  {sel[4]:.0f} GB")
    print(f"[thor] torch      : {torch.__version__}   CUDA: {torch.version.cuda}")
    print(f"[thor] arch_list  : {torch.cuda.get_arch_list()}")

    if "thor" in sel[1].lower():
        print("[thor] *** WARNING: selected the integrated Thor iGPU, NOT the RTX PRO 6000. "
              "Use --gpu-index for the discrete card (see the cuda:N lines above), "
              "or export CUDA_VISIBLE_DEVICES=<n>. ***")
    if not any(a.startswith(f"sm_{sel[2]}") for a in torch.cuda.get_arch_list()):
        print(f"[thor] *** WARNING: this PyTorch build may NOT support sm_{sel[2]}{sel[3]}. "
              f"If you hit 'no kernel image is available', install a CUDA 12.8 / cu128 "
              f"build (see setup_thor.sh). ***")
...
# --------------------------------------------------------------------------- #
def trainer_amp(args, config, model, device, precision, compile_model=False):
    from torch.utils.data import DataLoader
    from tensorboardX import SummaryWriter

    model.to(device, memory_format=torch.channels_last_3d)
    if compile_model and device.type == "cuda":
        print("[thor] torch.compile: compiling model — one-time cost of a few "
              "minutes on the first training step (fuses kernels for Blackwell)…")
        model = torch.compile(model)
    # a compiled model's state_dict keys get a '_orig_mod.' prefix; save from the
    # underlying module so checkpoints load cleanly into an uncompiled model.
    ...
    for epoch in range(args.max_epochs):
        ...
        if metric > best_metric:
            ...
            torch.save(base_model.state_dict(), os.path.join(args.save_path, "best_metric_model.pth"))
...
def main():
    ...
    ap.add_argument("--gpu-index", type=int, default=None,
                    help="CUDA device index to use (see the cuda:N lines printed at startup). "
                         "Default: auto-pick the discrete RTX PRO 6000 over the Thor iGPU.")
    ap.add_argument("--compile", action="store_true",
                    help="Enable torch.compile (Blackwell kernel fusion; one-time compile cost, "
                         "then faster steps). AMP loop only.")
    ...
    device = report_device(a.gpu_index)
    ...
    for split in ("train", "val"):
        ...
        if n == 0:
            sys.exit(f"[ERROR] {d} missing or empty. Run preprocess_to_npz_parallel.py first.")
    ...
    if a.repo_trainer:
        ...
        msg = trainer_amp(args, config, model, device, a.precision, compile_model=a.compile)
    print("[thor]", msg)
